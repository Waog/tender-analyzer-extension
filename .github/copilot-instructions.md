...
# Purpose of this repository / project

This Project is a chrome extension build with WXT. It's purpose is to alow the user to analyze public tenders, by processing their websites and files (zip, pdf, html, word, excel, openOffice, etc.), concatenating the content into one big LLM prompt, open a new browser tab and insert the prompt into an LLM chat.
...
When you are in chat mode: Prefer using the VSCode tools you have access to over custom command line execution. E.g. access VSCodes Testing Extension, instead of executing `npm test`, use VSCodes Search extension instead of `grep`, etc.
